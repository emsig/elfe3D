...
The mesh generator `TetGen` and the solver `MUMPS` are open source packages. Installation instructions are given below. Note that additional libaries have to be installed during the installation of `MUMPS`. Installation instructions are given below and in the `MUMPS` Makefile.
